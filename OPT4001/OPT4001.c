...
int opt4001_write_register(OPT4001 *sensor, uint8_t reg, uint16_t value) {
    ...
    return write(sensor->i2c_fd, buf, 3) == 3 ? 0 : -1;
}

int opt4001_read_register(OPT4001 *sensor, uint8_t reg, uint16_t *value) {
    if (write(sensor->i2c_fd, &reg, 1) != 1) return -1;

    uint8_t buf[2];
    if (read(sensor->i2c_fd, buf, 2) != 2) return -1;
...
int opt4001_read_raw(OPT4001 *sensor, uint32_t *raw, uint8_t *exp) {
    ...
    write(sensor->i2c_fd, start_config, 3);
    usleep(200000);

    uint8_t reg = 0x00;
    write(sensor->i2c_fd, &reg, 1);

    uint8_t data[2];
    read(sensor->i2c_fd, data, 2);
